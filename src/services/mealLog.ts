...
import { FoodItem, FoodLogItem, MealType } from "@/types";

const mealFieldByType: Record<MealType, "breakfast_kcal" | "lunch_kcal" | "dinner_kcal" | "snacks_kcal"> = {
...
export async function addFoodToDiary(userId: string, food: FoodItem | FoodLogItem, mealType: MealType) {
...
export async function updateFoodLogItem(userId: string, itemId: string, previous: EditableFoodLog, next: EditableFoodLog, mealType: MealType) {
  const userRef = doc(db, "users", userId);
...
function parsePortion(serving: string) {
...
export async function deleteFoodLogItem(userId: string, itemId: string, previous: EditableFoodLog, mealType: MealType) {
  const userRef = doc(db, "users", userId);
  const historyRef = doc(collection(userRef, "meal_history"), itemId);
  const todayId = new Date().toISOString().slice(0, 10);
  const mealField = mealFieldByType[mealType];
...
function macroDelta(previous: EditableFoodLog, next: EditableFoodLog) {
  return {
...
async function applyMealDelta(
  ...
  todayId: string,
  mealField: "breakfast_kcal" | "lunch_kcal" | "dinner_kcal" | "snacks_kcal",
