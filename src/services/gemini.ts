...
export async function estimateFoodFromImage(base64: string, mimeType = "image/jpeg"): Promise<GeminiFoodEstimate> {
  const prompt = [
...
export async function identifyFoodsFromImage(base64: string, mimeType = "image/jpeg"): Promise<string[]> {
  const prompt = [
