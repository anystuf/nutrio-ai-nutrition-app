...
export function PoseWorkoutCamera({ exercise, reps, calories, saving, onRep, onClose, onSave }: Props) {
  const videoRef = useRef<any>(null);
  const detectorRef = useRef<any>(null);
  const rafRef = useRef<number | null>(null);
  const streamRef = useRef<any>(null);
  const stageRef = useRef<"ready" | "down">("ready");
  const [status, setStatus] = useState(Platform.OS === "web" ? "Starting camera..." : "Native ML Kit pose module is required.");
  const [kneeAngle, setKneeAngle] = useState(0);
  const [stage, setStage] = useState<"ready" | "down">("ready");
  ...
  useEffect(() => {
    ...
    function updateSquatCounter(angle: number) {
...
function estimateSquatAngle(keypoints: Keypoint[]) {
...
function angleForSide(keypoints: Keypoint[], side: "left" | "right") {
...
function findPoint(keypoints: Keypoint[], name: string) {
...
const styles = StyleSheet.create({
  ...
  webVideo: { position: "absolute", left: 0, right: 0, top: 0, bottom: 0, width: "100%", height: "100%", objectFit: "cover", transform: "scaleX(-1)" } as any,
