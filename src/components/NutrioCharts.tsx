...
export function WeeklyBarChart({ points, goal, height = 220, color = "#C5E07E" }: BarChartProps) {
  const width = 330;
  const chartTop = 14;
...
export function MonthlyLineChart({ points, goal, height = 220, color = "#C5E07E" }: LineChartProps) {
  const width = 330;
  const left = 22;
  const right = 12;
  const top = 14;
...
function GaugeArc(props: { start: number; end: number; color: string; radius: number; stroke: number; cx: number; cy: number }) {
