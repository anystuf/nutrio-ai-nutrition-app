...
import { AppRoute, Recipe } from "@/types";
...
export function RecipesScreen({ onNavigate, favoritesOnly = false }: Props) {
  const [favorites, setFavorites] = useState<string[]>([]);

  useEffect(() => {
    void AsyncStorage.getItem(favoritesKey).then((value) => setFavorites(value ? JSON.parse(value) as string[] : []));
  }, []);
...
function RecipeCard({ recipe, active, onToggle, onOpen }: { recipe: Recipe; active: boolean; onToggle: () => void; onOpen: () => void }) {
  return (
