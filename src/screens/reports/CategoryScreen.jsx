...
import {
  ...
  greetingForHour,
  parseLegacyMealPlan,
  selectedMealCalories } from
"@/screens/reports/legacyMeals";
...
export function CategoryScreen({ user, onNavigate }) {
...
function ValueIntakeGrid({ nutrition }) {
  const rows = [
...
function ReportActionGrid({ onNavigate }) {
  return (
...
function MetricCard({ title, icon, label, value, color }) {
  return (
...
function Nutrient({ label, value, total, color }) {
  return (
...
function MealSummary({ title, value, items, color }) {
