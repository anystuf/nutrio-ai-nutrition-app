...
export function parseLegacyMealPlan(data) {
  const record = data ?? {};
  return {
    breakfast: [
...
export function calculateLegacyTotals(plan, data) {
  const record = data ?? {};
...
export function calculateLegacyNutrition(totals) {
...
export function activeMealLabel(hour) {
  if (hour > 18 && hour < 25) return "Supper time";
  if (hour > 12 && hour < 19) return "Lunch time";
  return "Breakfast time";
}

export function selectedMealCalories(hour, nutrition) {
...
export function buildLegacyMealUpdate(plan) {
