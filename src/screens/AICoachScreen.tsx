...
import {
  ...
  scriptedReply
} from "@/features/aiCoach/aiCoachScripts";
...
export function AICoachScreen({ user, onNavigate }: Props) {
  ...
  useEffect(() => {
    ...
    return onSnapshot(q, (snap) => {
      setMessages(snap.docs.map((docSnap) => ({ id: docSnap.id, ...docSnap.data() } as Message)));
    });
  }, [user.uid]);
  ...
  async function addBotMessage(nextText: string, quickReplies: string[] = []) {
