...
export function InsightsScreen({ user }) {
  const [profile, setProfile] = useState(null);
  const [logs, setLogs] = useState([]);
  const [period, setPeriod] = useState("week");

  useEffect(() => {
    return onSnapshot(doc(db, "users", user.uid), (snap) => setProfile(snap.exists() ? snap.data() : null));
  }, [user.uid]);
  ...
  useEffect(() => {
    return onSnapshot(collection(db, "users", user.uid, "daily_logs"), (snap) => {
      ...
      map((docSnap) => {
        ...
        return { date: dateIdFromValue(data.date, docSnap.id), ...data };
      }).
...
function OverviewCard({ eaten, goal, remaining }) {
...
function WeeklyChartCard({ points, goal }) {
  return (
    <View style={styles.card}>
...
function MonthlyChartCard({ points, goal }) {
  return (
    <View style={styles.card}>
...
function MacroCircle({ label, value, goal, color }) {
...
function Legend({ range, label, color }) {
  return (
...
function dateIdFromValue(value, fallback) {
  if (typeof value === "string" && /^\d{4}-\d{2}-\d{2}/.test(value)) return value.slice(0, 10);
  if (value && typeof value === "object" && "toDate" in value && typeof value.toDate === "function") {
    return toDateId(value.toDate());
  }
