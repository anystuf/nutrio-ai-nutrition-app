...
import { AppRoute, DailyLog, MealType, UserProfile } from "@/types";
...
type MealField = "breakfast_kcal" | "lunch_kcal" | "dinner_kcal" | "snacks_kcal";

const meals: Array<{ type: MealType; field: MealField; color: string; targetRatio: number; icon: keyof typeof Ionicons.glyphMap }> = [
...
export function HomeScreen({ user, onNavigate }: Props) {
  ...
  useEffect(() => {
    return onSnapshot(doc(db, "users", user.uid, "daily_logs", selectedDateId), (snap) => {
      setDailyLog(snap.exists() ? { date: selectedDateId, ...snap.data() } as DailyLog : null);
    });
  ...
  const computed = useMemo(() => {
    ...
    const mealValues = meals.reduce<Record<MealType, number>>((acc, meal) => {
...
function MacroCircle({ label, value, target, color }: { label: string; value: number; target: number; color: string }) {
  return (
...
function Burned({ label, value, icon }: { label: string; value: number; icon: keyof typeof Ionicons.glyphMap }) {
  return (
...
function numberOrFallback(...values: Array<number | undefined>) {
